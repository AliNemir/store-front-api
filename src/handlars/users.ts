...
async function show(req: Request, res: Response) {
    try {
        ...
        if (permession) {
            try {
                const result = await user_obj.show(parseInt(req.params.id))
                res.status(200).json(result)
            } catch (e) {
                res.status(400).json(`${e}`)
            }
...
async function delete_(req: Request, res: Response) {
    ...
    if (permession) {
        try {
            const result = await user_obj.delete(parseInt(req.params.id))
            res.status(200).json(result)
        } catch (e) {
            res.status(400).json(`${e}`)
        }
...
async function login(req: Request, res: Response) {
    try {
        ...
        const result = await user_obj.auth(username, password)
        if (result != null) res.status(200).send('succeed')
